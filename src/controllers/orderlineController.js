...
//Will never need to get all lines untied to an order.
...
//Get lines for a specific order using orderId. 
const getLinesByOrderId = async (req, res, next) => {
    let id = req.params.id;
    let unitId = req.params.unitId; //Will need to filter by unitId
    const lines = await db.orderline.findAll({ where: { OrderId: id, UnitId: unitId, Status: "Allocated" } });
    res.send(lines);
